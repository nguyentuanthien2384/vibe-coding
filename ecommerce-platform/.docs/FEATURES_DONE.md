...
- **Đồng Bộ Link Ảnh Chuyên Mục trên Admin Dashboard ([category-table-row.tsx](file:///d:/vibe_coding/ecommerce-platform/app/dash/my-app/features/categories/components/category-table-row.tsx) & [image-uploader.tsx](file:///d:/vibe_coding/ecommerce-platform/app/dash/my-app/components/ui/image-uploader.tsx)):** Tự động nối origin backend (`http://localhost:3001`) cho các đường dẫn ảnh upload dạng tương đối (`/uploads/images/...`), đồng thời hỗ trợ xem trước và hiển thị hoàn hảo cả biểu tượng Emoji và ảnh Icon mà không bị lỗi tải ảnh hay văng URL rác.
